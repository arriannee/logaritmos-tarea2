...
void dijkstraWithFibonacci(Graph& graph, int root) {
    ...
    while (!fibHeap.empty()) {
        ...
        for (const auto& edge : graph.nodes[u->id].edges) {
            int v = edge.end; // Nodo vecino
...
void testDijkstraWithFibonacciGrande() {
    int numNodos = 100; // 100 nodos
    int numAristas = 200; // 200 aristas
    Graph grafo = construirGrafo(numNodos, numAristas);

    // Ejecutar Dijkstra desde el nodo 0
    dijkstraWithFibonacci(grafo, 0);

    // No vamos a reconstruir un camino específico aquí ya que es un grafo grande y aleatorio
}

int main() {
    testDijkstraWithFibonacciGrande(); // Llamar a la función de prueba
    cout << "FIN";
    return 0;
}
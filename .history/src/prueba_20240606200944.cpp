...
struct FibonacciHeap {
    ...
    if (x == nullptr || y == nullptr) {
        cerr << "Error: Null pointer encountered in cut." << endl;
        return;
    }
    ...
    x->left = minNode;
    if (minNode == nullptr) {
        cerr << "Error: minNode is null in cut." << endl;
        return;
    }
    ...
    cout << "cut: Node " << x->id << " cut from node " << y->id << " and added to the root list." << endl;
}
...
void consolidate() {
    ...
    for (FibonacciNode* w : rootNodes) {
        ...
        while (A[d] != nullptr) {
            ...
            cout << "Comparing nodes x (id: " << x->id << ", key: " << x->key << ") and y (id: " << y->id << ", key: " << y->key << ")" << endl;
            if (x->key > y->key) {
                swap(x, y);
            }
            link(y, x);
...
void link(FibonacciNode* y, FibonacciNode* x) {
    ...
    cout << "Linking node y (id: " << y->id << ", key: " << y->key << ") to node x (id: " << x->id << ", key: " << x->key << ")" << endl;
    ...
    ++x->degree;
    y->mark = false;
    cout << "link: Node " << y->id << " linked to node " << x->id << endl;
}

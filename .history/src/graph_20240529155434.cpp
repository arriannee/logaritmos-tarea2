...
Graph construirGrafo(int numNodos, int numAristas) {
    ...
    uniform_real_distribution<double> distribucion(0.01, 1.0); // Cambia el rango para asegurar que los pesos estén en (0, 1]
...
void imprimirGrafo(const Graph& grafo) {
    cout << "Nodos y Aristas del Grafo: \n";
    for (const auto& nodo : grafo.nodes) {
        cout << "Nodo " << nodo.first << ": \n";
        for (const auto& vecino : nodo.second.matrix) {
            for (const Edge& arista : vecino.second) {
                cout << "  -> Nodo " << arista.end << " con peso " << arista.weight << "\n";
            }
        }
    }
    cout << "Fin del Grafo \n";

...
public:
    ...
    void dijkstra(int src) {
        ...
        FibHeap pq;
        std::vector<FibNode*> nodePointers(V, nullptr);

        dist[src] = 0;
        nodePointers[src] = pq.insert(src, 0);

        for (int i = 0; i < V; ++i) {
            if (i != src) {
                nodePointers[i] = pq.insert(i, std::numeric_limits<double>::infinity());
            }
        }
        ...
        while (!pq.isEmpty()) {
            ...
            if (u == nullptr) {
                std::cerr << "Error: Nodo extraído es nullptr" << std::endl;
                break;
            }
            ...
            for (const auto& neighbor : adj[u_key]) {
                ...
                if (dist[u_key] + weight < dist[v]) {
                    ...
                    prev[v] = u_key;
                    if (nodePointers[v] != nullptr) {
                        pq.decreaseKey(nodePointers[v], dist[v]);
                        std::cout << "Actualizando distancia del nodo " << v << " a " << dist[v] << std::endl;
                    } else {
                        std::cerr << "Error: nodePointers[" << v << "] es nullptr" << std::endl;
                    }
                }
            }

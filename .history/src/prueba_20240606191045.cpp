...
pair<vector<double>, vector<int>> dijkstraWithFibonacciHeap(Graph& graph, int raiz) {
    ...
    FibonacciHeap Q;
    vector<FibonacciNode*> allNodes; // Vector para almacenar todos los nodos de Fibonacci
    ...
    for (const auto& node : graph.adjList) {
        ...
        allNodes.push_back(fibNode); // Guardar el puntero para liberarlo más tarde
        Q.insert(fibNode);
    }
    ...
    // Liberar memoria de todos los nodos restantes en allNodes
    for (auto& node : allNodes) {
        delete node;
    }
...
void testDijkstraWithFibonacci() {
    ...
    Graph grafo1 = construirGrafo(10, 12); // 1024 nodos, 4096 aristas

...
void testDijkstraWithFibonacci(int i, int j) {
    ...
    if (timingFile.is_open()) {
        for (int experiment = 0; experiment < numExperiments; ++experiment) {
            ...
            Graph grafo = crearGrafo(i, j); // Asumiendo que esta función ya está definida
...
int main() {
    vector<pair<int, int>> configuraciones = {{10, 16}, {10, 18}, {10, 20}, {10, 22}, {12, 16}, {12, 18}, {12, 20}, {12, 22}, {14, 16}, {14, 18}, {14, 20}, {14, 22}};
    
    for (const auto& config : configuraciones) {
        int i = config.first;
        int j = config.second;
        cout << "Ejecutando pruebas para v = 2^" << i << " y e = 2^" << j << "\n";

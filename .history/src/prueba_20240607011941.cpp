...
// Función para comparar los resultados de dos implementaciones de Dijkstra
bool compararResultados(const pair<vector<double>, vector<int>>& resultado1, const pair<vector<double>, vector<int>>& resultado2) {
    return resultado1.first == resultado2.first && resultado1.second == resultado2.second;
}
...
int main() {
    ...
    for (const auto& config : configuraciones) {
        ...
        Graph grafo = construirGrafo(i, j);

        auto resultadoFibonacci = dijkstraWithFibonacciHeap(grafo, 0);
        auto resultadoMinHeap = dijkstraWithMinHeap(grafo, 0);

        bool sonIguales = compararResultados(resultadoFibonacci, resultadoMinHeap);

        if (sonIguales) {
            cout << "Los resultados son iguales.\n";
        } else {
            cout << "Los resultados son diferentes.\n";
        }
    }
